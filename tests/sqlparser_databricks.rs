...
fn parse_set_session_variable() {
    databricks().verified_stmt("SET VAR process_start_at = 1");
    databricks().verified_stmt("SET VARIABLE process_start_at = 1");
    databricks().verified_stmt("SET VAR process_start_at = DEFAULT");
    databricks().verified_stmt("SET VARIABLE process_start_at = DEFAULT");
    databricks().verified_stmt("SET VAR process_start_at = (SELECT 1)");
    databricks().verified_stmt(
        r#"SET VAR process_start_at = (SELECT date_trunc('hour', to_timestamp('${yyyy-MM-dd HH:mm:ss}', 'yyyy-MM-dd HH:mm:ss') - INTERVAL 4 HOUR))"#,
    );

    match databricks().verified_stmt("SET VAR process_start_at = 1") {
    ...
    match databricks().verified_stmt("SET VARIABLE process_start_at = DEFAULT") {
        Statement::Set(Set::SetSessionVariable {
            keyword,
            variable,
            value,
    ...
    match databricks().verified_stmt("SET process_start_at = 1") {
